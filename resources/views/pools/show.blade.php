        ...
        <div class="col-lg-10">
            ...
            @auth
                @if(Auth::user()->role === 'admin')
                    <form action="{{ route('pools.destroy', $pool) }}" method="POST" class="mt-3" onsubmit="return confirm('آیا از حذف استخر مطمئن هستید؟')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger w-100">
                            <i class="fas fa-trash me-1"></i> Delete Pool (Admin)
                        </button>
                    </form>
                @endif
            @endauth
